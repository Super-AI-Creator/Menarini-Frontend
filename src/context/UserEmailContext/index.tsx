...
export const EmailContextProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  ...
  useEffect(() => {
    ...
    socket.on('new_email', (data) => {
      console.log("New email event received with data:", data);
      if (!data) {
        console.log('No data received in new_email event');
        return;
      }
  ...
  return (
    <EmailContext.Provider value={{ emails, selectedEmail, setSelectedEmail, deleteEmail, toggleStar, toggleImportant, setFilter, filter, searchQuery, setSearchQuery, startOCR, newPO, EmailError, DocumentError, getIntervention ,multiDocIntervention,supplierIntervention, update_multi_doc_type, setUpdatedSupplierName, supplier_name_list}}>
      {children}
