...
import { EmailContext } from "src/context/UserEmailContext";
...
const EmailList = ({ openMail }: MailListProps) => {
  ...
  const filteredEmails = searchQuery
    ...
    : emails.filter((email: { [x: string]: any; starred: any; label: any; }) => {
      ...
      } else if (['Complete', 'Pending'].includes(filter as string)) {
                    ...
                    <div className="flex gap-2 items-center" >
                      {email.starred ? (
                        ...
                        <Icon icon='solar:star-line-duotone'
                          height="16"
                          className="text-dark dark:text-bodytext"
                          onClick={() => toggleStar(email.id)}
                        />
                      )}
                      {email.important ? (
                      ...
                      {checkedItems[email.id] && <Icon icon="solar:trash-bin-minimalistic-outline" height="17"
                        className="text-dark dark:text-bodytext" onClick={() => handleDelete(email.id)} />}
                    </div>
                    <p className="text-xs  font-medium mt-0.5">{email.time} </p>
                  </div>
                </div>
              </div>
            </div>
